...
def montar_pesquisa(d):
    ...
    if mais:
        p["mais_filtros"] = mais
    # 'limite' vai DENTRO de pesquisa (1..1000) conforme doc oficial v5
    try:
...
def gerar_e_baixar(api_key, nome, pesquisa, tipo, total_linhas, log):
    # A API v5 SÓ aceita "csv" (doc oficial: "no momento temos apenas o csv").
    # Mandar "excel" causa HTTP 400. Forçamos csv sempre.
    ...
    if r.status_code not in (200, 201, 202):
        # Mostra a mensagem real da API para diagnóstico preciso
        try:
            detalhe = r.json()
    ...
    for t in range(1, POLL_MAX + 1):
        try:
            ...
            pass
        # Intervalo progressivo: rápido no começo, mais espaçado depois
...
def adm_get_key():
    k = db.get_api_key()
    # Mostra só os últimos 4 caracteres por segurança
...
def api_solicitacoes():
    """Lista as solicitações de arquivo já feitas (não custa crédito)."""
    key = db.get_api_key()
    if not key:
...
def api_baixar_solicitacao(uuid):
    """Baixa o arquivo de uma solicitação já processada (sem custo extra)."""
    key = db.get_api_key()
    if not key:
...
<div class="painel active" id="p1">
 ...
 <div class="testbar">
  ...
  <div><b>Modo teste rápido</b>
   <small>Marque para buscar só 5 CNPJs e validar os filtros
...
async function captar(){
 ...
 const tl=$('modo_teste').checked?5:$('total_linhas').value;
 const body={cnae:$('cnae').value,situacao:$('situacao').value,
  ufs:$('ufs').value,municipios:$('municipios').value,
  ultimos_dias:$('ultimos_dias').value,tipo:$('tipo').value,
  total_linhas:tl,com_telefone:$('com_telefone').checked,
  com_email:$('com_email').checked,somente_mei:$('somente_mei').checked,
